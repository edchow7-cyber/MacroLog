...
async function handleExport(url, env) {
  ...
  // Full log export — fetch all date keys, optionally filtered by days
  const daysParam = parseInt(url.searchParams.get('days') || '0');
  const list = await env.MACROLOG.list({ prefix: 'log:' });
  const allRows = [];
  const cutoff = daysParam > 0
    ? new Date(Date.now() - daysParam * 86400000).toLocaleDateString('en-CA')
    : null;
  for (const key of list.keys) {
    const dateStr = key.name.replace('log:', '');
